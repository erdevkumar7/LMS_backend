...
exports.getCourseByUserIdForPushNotification = async (req, res) => {
  const user_id = req.params.id;
  ...
  try {
    ...
    // Now, modify the combinedArray to include course and module details

    const combinedArray = await Promise.all(
      getEnrollData.map(async (course) => {
        ...
        // Sort sessions by live_date in ascending order
        sessions.sort((a, b) => new Date(a.live_date) - new Date(b.live_date));

        // Calculate time remaining for each session
        const sessionsWithTimeRemaining = sessions.map((session) => {
          const liveDate = new Date(session.live_date);
          const timeRemaining = liveDate - currentDate;
        ...
        // Find the session with the smallest live_date and time remaining of 15 minutes
        const smallestSession = sessionsWithTimeRemaining.find(
          (session) => session.timeRemaining <= 15 * 60 * 1000  // 10-15 minutes in milliseconds
        );
        return {
          smallestSession,
        };
